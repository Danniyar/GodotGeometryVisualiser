...
func _ready():
	...
	points[3].position.x = 0.05
	points[3].position.y = 0.1
	points[3].position.z = (points[0].posZ+points[1].posZ+points[2].posZ)/3
	...
	get_node('/root/Node3D/VectorsManager')._on_create_pressed(points[0], points[3])
	get_node('/root/Node3D/VectorsManager')._on_create_pressed(points[1], points[3])
	get_node('/root/Node3D/VectorsManager')._on_create_pressed(points[2], points[3])
	
	name = "Pyramide " + points[0].name + points[1].name + points[2].name + points[3].name
	updateData()
...
func _on_width_value_changed(value):
	...
	if points[3] != null:
		points[3].position.x = (points[0].posX+points[1].posX+points[2].posX)/3
		points[3].posX = points[2].position.x
		points[3].updateData()
